// Página: Login
// Propósito: Tela de login e registro rápido (formulários).
...
export default function Login() {
  ...
  const onlyLogin = params.get('onlyLogin') === '1'
  ...
  const passwordValidation = modo === 'register' && password ? validatePassword(password) : null

  // show/hide password states with auto-hide
  const [showPwdLogin, setShowPwdLogin] = useState(false)
  const [showPwdRegister, setShowPwdRegister] = useState(false)
  const loginHideRef = useRef(null)
  const registerHideRef = useRef(null)

  useEffect(() => {
    return () => {
      if (loginHideRef.current) clearTimeout(loginHideRef.current)
      if (registerHideRef.current) clearTimeout(registerHideRef.current)
    }
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modo === 'login') {
        const { data } = await flaskClient.post('/api/auth/login', { email, password })
  ...
  return (
    <div className="login-page">
      ...
      <div className="login-card">
        ...
        <p className="subtitle">Conecte-se ao mercado de trabalho</p>
        ...
        <div className="form-container">
          { !onlyLogin ? (
            <div className={`form-slide ${modo === 'login' ? 'login-active' : 'register-active'}`}>
              {/* Formulário de Login */}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (showPwdLogin) {
                        ...
                        if (loginHideRef.current) clearTimeout(loginHideRef.current)
                        loginHideRef.current = setTimeout(() => setShowPwdLogin(false), 5000)
                      }
                    }}
                  >{showPwdLogin ? 'Ocultar' : 'Mostrar'}</button>
                </div>
              ...
              <form onSubmit={handleSubmit} className="form-panel">
                ...
                <div className="pwd-row">
                  ...
                  <button
                    type="button"
                    className="show-pwd-btn"
                    aria-pressed={showPwdRegister}
                    onClick={() => {
                      if (showPwdRegister) {
                        setShowPwdRegister(false)
                        if (registerHideRef.current) clearTimeout(registerHideRef.current)
                      } else {
                        setShowPwdRegister(true)
                        if (registerHideRef.current) clearTimeout(registerHideRef.current)
                        registerHideRef.current = setTimeout(() => setShowPwdRegister(false), 5000)
                      }
                    }}
                  >{showPwdRegister ? 'Ocultar' : 'Mostrar'}</button>
                </div>
            ...
            <div className="form-slide login-active">
              <form onSubmit={handleSubmit} className="form-panel">
                <input
                  type="email"
                  placeholder="Email"
